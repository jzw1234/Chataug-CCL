...
# from meshed_memory_encoder_decoder import EncoderDecoder
# from visual_extractor import VisualExtractorDenseNet121 as VisualExtractor
...
class CoNTGenerator(nn.Module):
    ...
    def __init__(self, PTM, model_name, pad_id, args):
        ...
        if self.PTM == "pegasus":
            ...
            self.generator = T5ForConditionalGeneration.from_pretrained('pretrained_weights/xsum/t5')#EncoderDecoder#
    ...
    def forward(self,  src_inp, target_inp, target_outp):#images,
        """
        ...
        batch_size = src_inp.size(0)#2
        src_pad_mask = ~(src_inp == self.pad_id)#padding_mask
        ...
        y0 = lm_logits.view(-1, self.vocab_size)
        nll_loss = self.loss_fct(y0, target_outp.view(-1))

        file = open('results/nll_loss.txt', mode='a')
        file.write(str(nll_loss.item()))
        file.write('\n')

        # y1 = torch.argmax(lm_logits, dim=2)
        # val_gts, val_res = [], []
        # for i in range(y1.shape[0]):
            # prediction = [tokenizer.decode(y1[i]).replace('</s>','')]
            # ground_truth = [tokenizer.decode(target_outp[i]*tgt_pad_mask[i]).replace('</s>','').replace('<pad>','')]
            # source=tokenizer.decode(src_inp[i]).replace('<pad>','').replace('</s>','')
            # val_met = self.compute_scores({0: [ground_truth[0]]},
            #                        {0: [prediction[0]]})
            # if val_met['BLEU_1']<=0.2:#修改这个值
            #     file = open('results/used_for_ChatGPT.txt', mode='a')
            #     file.write(source)
            #     file.write('\n')
        ...
        # prepare contrastive learning#[2,12,30]
        samples_from_batch = target_inp[None, :, :].repeat(batch_size, 1, 1)#[2,29]->[2,2,29]
        ...
        samples_all = torch.cat([cand_ids, samples_from_batch], dim=1) #[2,14,30] # batch x total_sample_num x seq_len
        ...
        for sample_idx in range(sampled_indices.size(-1)):
            sampled_input_dec = sampled_input[:, sample_idx, :]#[2,14,30]
        ...
        file = open('results/cl_loss.txt', mode='a')
        file.write(str(cl_loss.item()))
        file.write('\n')
